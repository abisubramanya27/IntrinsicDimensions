...
import sys
if '/content/early-stopping-pytorch/' not in sys.path:
...
def main_fn(MODEL_NAME, DATASET, CONFIG, BATCH_SIZE, MAX_LENGTH, NUM_TRAIN_SAMPLES, NUM_EVAL_SAMPLES, NUM_LABELS, NUM_EPOCHS,
            LR, ID=0, said=False, wandb_log=True, output_dir=None, MODEL_PATH=None, stage=1):
    said_str = "_SAID" if said == 1 else "_HAID2" if said == 2 else ''
    run_name = f"{MODEL_NAME}_ID{ID}_lr{LR}_ml{MAX_LENGTH}"+said_str if ID>0 else f"{MODEL_NAME}_baseline_lr{LR}_ml{MAX_LENGTH}_stage{stage}"
    ...
    if wandb_log:
        ...
        run = wandb.init(reinit=True, config=config, project=f'mbert-256-xnli-multi-lang', entity='iitm-id', name=run_name, resume=None)
    ...
    if output_dir!=None and wandb_log:
        ...
        artifact.add_file(output_path, name=f'best_model.pth')
...
def zeroshot_fn(model, MODEL_NAME, DATASET, lang, db, BATCH_SIZE, MAX_LENGTH, LR, stage=1):
    loss_fn = torch.nn.CrossEntropyLoss()
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    # PR = float(model_dir[model_dir.lower().index("pr")+2 : ])
    run_name = f"{MODEL_NAME}_baseline_lr{LR}_ml{MAX_LENGTH}_stage{stage}"
    config = {
        'model_name': MODEL_NAME,
        'dataset': DATASET,
        'language': lang,
    ...
    run = wandb.init(reinit=True, config=config, project=f'mbert-256-xnli-multi-lang-zeroshot', entity='iitm-id', name=run_name, resume=None)
...
OUTPUT_DIR = '/nlsasfs/home/ai4bharat/shubhamr/indic-analysis/container/multi_lang_checkpoints/2/'

for ID in [0]:
    main_fn(MODEL_NAME, DATASET, CONFIG, BATCH_SIZE, MAX_LENGTH, NUM_TRAIN_SAMPLES, NUM_EVAL_SAMPLES, NUM_LABELS, NUM_EPOCHS,
            ID_lr_dict[int(ID)], int(ID), said=0, wandb_log=True, output_dir=OUTPUT_DIR, MODEL_PATH='/nlsasfs/home/ai4bharat/shubhamr/indic-analysis/container/checkpoints_mbert_xnli/bert-base-multilingual-cased_baseline_lr3e-05_ml256/epoch_3.pth', stage=2)
    model = load_best_model(os.path.join(MODEL_PATH, 'best.pth'))
    for lang in ['en', 'ar', 'bg', 'de', 'el', 'es', 'fr', 'hi', 'ru', 'sw', 'th', 'tr', 'ur', 'vi', 'zh']:
      ...
      zeroshot_fn(model, MODEL_NAME, DATASET, lang, db, BATCH_SIZE, MAX_LENGTH, LR, stage=2)
