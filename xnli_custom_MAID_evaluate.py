...
def zeroshot_fn(MODEL_NAME, DATASET, lang, CONFIG, db, BATCH_SIZE, MAX_LENGTH, output_path, said=2, ID_reqd=None):
    ...
    for model_dir in os.listdir(output_path):
        ...
        config = {
            ...
            'ID': ID,
            # 'prune_frac': PR,
            'mode': 'NIL' if ID == 0 else 'DID' if not said_file else 'SAID' if said_file == 1 else 'MAID2',
            'lr_scheduler': 'linear',
            'optim': 'Adam',
            'beta1': 0.9,
            'beta2': 0.999,
        ...
        run = wandb.init(reinit=True, config=config, project=f'mbert-{DATASET}-{CONFIG}-{MAX_LENGTH}-HAID-zeroshot', entity='iitm-id', name=run_name, resume=None)
...
output_dir = os.getenv("OUTPUT_DIR", "/home/indic-analysis/container/checkpoints_mbert_maid_en/")
